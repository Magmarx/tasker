...
Ext.define('Ext.ux.plugin.Json', {
    ...
    context: null,
    instanceId: null,
    _instanceId: Ext.guid ? Ext.guid() : null,
    ...
    isEmpty: function(B) {
        ...
        if (B instanceof Array) {
            ...
            if (typeof(B) === 'object') {
                for (var C in B) {
                    if ((!me.useHasOwn || B.hasOwnProperty(C)) && (!me.jsonId || C.indexOf(this.jsonId) != 0)) {
                        return false;
                    }
                }
            } else {
    ...
    load: function(url, obj) {
        ...
        if (obj && obj instanceof Ext.container.Container) {
            return me.load(url, Ext.bind(function(callback) {
                me.apply(obj, callback);
            }, me));
        } else {
            if (typeof(obj) === 'function') {
                Ext.Ajax.request({
                    ...
                    callback: function(CallBack, Scope, Response) {
                        try {
                            if (Scope) {
                                var C = me.decode(Response.responseText);
                            ...
                            me.fireEvent('error', 'load', failError);
                        }
                    },
                    scope: me
                });
                ...
                return me.decode((typeof(url) === 'object') ? me.syncContent(url.url, url.nocache == undefined ? me.nocache : url.nocache) : me.syncContent(url.url, me.nocache));
            }
        }
    },
    ...
    set: function(scope, content, opciones) {
        ...
        if (content) {
            ...
            if (typeof(content) === 'string') {
                content = me.decode(content, opciones);
            }
                ...
                if (key == 'required_js') {
                    if (content[key]) {
                        ...
                        for (var I = 0; I < A.length; I++) {
                            ...
                            if (!me.scriptLoader(A[I], opciones.nocache)) {
                                var J = new Error('Failed to load javascript ' + A[I]);
                                if (me.fireEvent('error', 'set', J)) {
                                    throw J;
                                }
                            }
                        }
                    ...
                    if (key == 'required_css') {
                        ...
                        if (key.indexOf('scope.') == 0) {
                            D = opciones.scope ? opciones.scope : me.getScope();
                        ...
                        try {
                            ...
                            if (opciones.ignoreError) {
                                ...
                                returns |= (me.fireEvent('error', 'set(' + E + ')', J) === false);
                            }
                        }
                    }
                }
            ...
            if (!me.useHasOwn || jCode.hasOwnProperty(item)) {
                if (item == 'items') {
                    if (jCode[item] instanceof Array) {
                        ...
                        jCode[item] = me.clean(jCode[item]);
                    }
                }
    ...
    editable: function(jCode) {
        if (typeof jCode === 'undefined' || jCode === null) return jCode;
        var code = jCode || {};
        var me = this,
            Ex = Ext,
            idPrefix = me._instanceId ? me._instanceId + '-' : '';
        //  console.log(code.field)
        if (typeof(code) !== 'object') {
            code = me.decode(jCode);
        }
        if (!me.jsonId) {
            return code;
        }
        if (code instanceof Array) {
            for (var x = 0; x < code.length; x++) {
                if (!code[x].id) {
                    code[x].id = Ex.id(code[x], idPrefix + code[x].xtype);
                } else {
                    var idTokens = code[x].id.split('-');
                    if (idTokens.length) {
                        code[x].id = idPrefix + idTokens[idTokens.length - 1];
                    }
                }
                code[x].configData = code[x].configData || {};
                if (!code[x].configData.referenceName) {
                    code[x].configData.referenceName = Ex.id(code[x], idPrefix + code[x].xtype);
                }
                code[x].configData.instanceId = me.instanceId;
                code[x].id = code[x].id.toUpperCase();
                code[x] = me.editable(code[x]);
            }
            return code;
        } else {
            if (!code.id) {
                code.id = Ex.id(code, idPrefix + code.xtype);
            } else {
                var idTokens = code.id.split('-');
                if (idTokens.length) {
                    code.id = idPrefix + idTokens[idTokens.length - 1];
                }
            }
            code.configData = code.configData || {};
            if (!code.configData.referenceName) {
                code.configData.referenceName = Ex.id(code, idPrefix + code.xtype);
            }
            code.configData.instanceId = me.instanceId;
            code.id = code.id.toUpperCase();
        }
        if (!code[me.jsonId]) {
            code[me.jsonId] = Ext.id();
            if (code.items) {
                code.items = me.editable(code.items);
            }
        }
        // console.log(code);
        return code;
    ...
    apply: function(jScope, jCode, B) {
        ...
        var me = this;
        try {
            A = this.jsonId ? me.editable(jCode) : jCode || {};
            if (typeof(A) !== 'object') {
                A = me.decode(jCode);
            }
            if (A && (A instanceof Array || typeof(A) === 'object')) {
                ...
                me.fireEvent('beforeapply', jScope, A);
            ...
            if (me.fireEvent('error', 'apply', E)) {
                throw E;
            }
        } finally {
            me.fireEvent('afterapply', jScope, A);
        }
        return A;
    },
    getRequiredClassNames: function(config, stack) {
        ...
        if (config instanceof Array) {
            for (var x = 0; x < config.length; x++) {
                ...
                if (!config[x].id) {
                    config[x].id = Ex.id(config[x], idPrefix + config[x].xtype);
                } else {
                    var idTokens = config[x].id.split('-');
                    if (idTokens.length) {
                        config[x].id = idPrefix + idTokens[idTokens.length - 1];
                    }
                }
                config[x].configData = config[x].configData || {};
                if (!config[x].configData.referenceName) {
                    config[x].configData.referenceName = Ex.id(config[x], idPrefix + config[x].xtype);
                }
                config[x].configData.instanceId = me.instanceId;
            ...
            if (!config.id) {
                config.id = Ex.id(config, idPrefix + config.xtype);
            } else {
                var idTokens = config.id.split('-');
                if (idTokens.length) {
                    config.id = idPrefix + idTokens[idTokens.length - 1];
                }
            }
            config.configData = config.configData || {};
            if (!config.configData.referenceName) {
                config.configData.referenceName = Ex.id(config, idPrefix + config.xtype);
            }
            config.configData.instanceId = me.instanceId;
    ...
    encode: function(objectDefinition, indent, keepJsonId, noLicense) {
        ...
        if (indent == 0) {
            var K = [],
                H = (!noLicense && this.licenseText) ? this.licenseText + '\n' : '';
    ...
    setJsonValue: function(A, C, D) {
        ...
        var B = new Ext.create('Ext.ux.plugin.Json', {
            jsonId: this.jsonId,
            nocache: true,
            evalException: false
        });
            ...
            if (content && typeof(content) === 'object') {
                ...
                if (content) {
                    try {
                        ...
                        var valueAux = this.decode(content, {
                            exceptionOnly: true,
                            scope: scope
                        });
                        if (objType) {
