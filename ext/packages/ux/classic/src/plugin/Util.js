...
Ext.define('Ext.ux.plugin.Util', {
    ...
    merge: function(item1, item2) {
        if (item1 instanceof Array && item2 instanceof Array) {
            ...
            var obj = {};
            for (var i in item1) {
                obj[i] = this.merge(item1[i], item2[i]);
            }
            for (var i in item2) {
                if (!obj[i]) obj[i] = item2[i];
            }
            return obj;
                                ...
                                data = setData(
                                    ...
                                    Ext.isIE
                                        ? el.options[j].attributes['value'].specified
                                            ? el.options[j].value
                                            : el.options[j].text
                                        : el.options[j].hasAttribute('value')
                                            ? el.options[j].value
                                            : el.options[j].text
                                );
                            }
    ...
    require: function(packages, options) {
        var _require = Ext.bind(function(packages, options) {
            ...
            for (var i = 0; i < packages.length; i++) {
                ...
                var uri =
                    dir +
                    (dir.length != 0 && dir.charAt(dir.length - 1) != '/' ? '/' : '') +
                    url.directory +
                    url.file +
                    (url.query ? '?' + url.query : '');
                if (url.ext == 'css') {
