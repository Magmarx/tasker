...
Ext.define('Ext.ux.widget.parameter.Dialog', {
    ...
    initComponent: function() {
        ...
        ExArray.each(initParameterVector, function(category) {
            ...
            var advancedTools = (category.proxy && searchItems.length) ? [{
                iconCls: Ext.fontBasePrefix + 'searchmore' + Ext.fontBasePostfix,
                border: true,
            ...
            var categoryFieldSet = Ex.widget('panel', categoryFieldsetConfig);

            ExArray.push(items, categoryFieldSet);
        });
        var parameterDialogForm = Ext.create('Ext.form.Panel', {
            layout: 'form',
            border: false,
            items: items
        });
        Ex.applyIf(me, {
            ...
            items: [parameterDialogForm],
            buttons: [{
                text: ExLocale.parameterSetupDialog.buttonText._continue,
                handler: function() {
                    var dialogForm = parameterDialogForm.getForm();
    ...
    getInvalidAlert: function(form, data, eMessage) {
        ...
        var errorGrid = Ext.widget('gridpanel', {
        ...
        var errorMessage = Ext.widget('container', {
            region: 'north',
            // heigth: 40,
        ...
        var errorWindow = Ext.widget('window', {
            modal: true,
