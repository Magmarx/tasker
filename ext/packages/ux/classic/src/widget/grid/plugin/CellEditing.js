...
Ext.define('Ext.ux.widget.grid.plugin.CellEditing', {
    ...
    instanceId: null, // set criteriaPanel
    context: null,
    validPrefix: [
        'Parameter',
        'DataSource'
    ...
    isValidValue: function(value) {
        var tokens,
            me = this;
        me.context = me.context || me.getCurrentContext(me.instanceId);
        if (typeof value === 'string') {
            if (value.indexOf('{{') !== -1 && value.indexOf('}}') !== -1) {
                ...
                // </debug>
                if (tokens.length === 3) {
                    if (me.validPrefix.contains(tokens[0].trim())) {
                        var fieldDataStoreName = tokens[0].toString() + 'DisplayStore',
                            // fieldDataStore = Ext.StoreManager.lookup(fieldDataStoreName),
                            fieldDataStore = me.context[fieldDataStoreName],
                            actualValueInDisplayStore = (fieldDataStore) ? fieldDataStore.query('dataIndex', tokens[2].toString(), false, false, true) : [];
