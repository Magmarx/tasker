...
Ext.define('Ext.ux.widget.grid.CriteriaPanel', {
    ...
    xtype: 'wheregrid',
    requires: ['Ext.ux.widget.grid.store.CriteriaStore', /**/
        ...
        'Ext.ux.widget.grid.config.CriteriaDialog' /**/
    ],
    // id: 'whereGrid',
    // show mode read panel editor and not show columns and buttons edit, add, and delete
    isReadOnly: true,
    instanceId: null,
    validPrefix: [
        'Parameter',
    ...
    initComponent: function() {
        ...
        whereGridCellEditing.on('displayadvanceddialog', function(rec, parameters) {
            var critWin = Ex.widget('criteriaconfig', {
                instanceId: me.instanceId,
                constrain: true,
                validPrefix: me.validPrefix,
            ...
            context.Container.add(critWin);
            critWin.show();
        });
        var getOpenedParenthesisRemaining = function(rowIndex) {
            var openParenthesisAbove = 0,
                closedParenthesisAbove = 0,
                closedParenthesisBelow = 0;
        ...
        var getClosedParenthesisRemaining = function(rowIndex) {
            var lastClosedParenthesisRow = -1,
                openParenthesisAbove = 0,
                closedParenthesisAbove = 0,
                openParenthesisBelow = 0,
                closedParenthesisBelow = 0;
            Ex.Array.each(whereStore.data.items, function(record, index) {
                if (record.get('parenthesisF')) {
                    if (index <= rowIndex) {
        ...
        var onParenthesisFClick = function(record, value, rowIndex) {
            if (!record.get('parenthesisF')) {
                if (getOpenedParenthesisRemaining(rowIndex)) {
                    ...
                    this.text = value ? ')' : '';
                    this.tooltip = value ? ELocale.buttonText._remove : ELocale.buttonText._add;
                } else {
        ...
        var fieldComboBox = Ex.widget('combobox', {
        ...
        if (!me.isReadOnly) {
            Ext.Array.push(configColumns,
                valueConfig, {
                    ...
                    iconCls: Ext.fontBasePrefix + 'edit' + Ext.fontBasePostfix,
                    altText: '...',
                            ...
                            if (typeof rec.get('value') === 'string') {
                                if (rec.get('value').indexOf('{{') !== -1 && rec.get('value').indexOf('}}') !== -1) {
                                    ...
                                    if (parameters.length === 3) {
                                        ...
                                        if (me.validPrefix.contains(parameters[0].trim())) {
                                            var fieldDataStoreName = parameters[0].toString() + 'DisplayStore',
                                                //    fieldDataStore = Ext.StoreManager.lookup(fieldDataStoreName),
                                                fieldDataStore = context[fieldDataStoreName],
                                                actualValueInDisplayStore = (fieldDataStore) ? fieldDataStore.query('dataIndex', parameters[2].toString(), false, false, true) : [];
                    ...
                    // hidden: args.complete? !args.complete: true,
                    iconCls: Ext.fontBasePrefix + 'delete' + Ext.fontBasePostfix,
                    altText: '...',
        ...
        Ex.applyIf(me, {
            ...
            plugins: (!me.isReadOnly) ? [
                whereGridCellEditing
            ] : null,
            tbar: (!me.isReadOnly) ? [{
                iconCls: Ext.fontBasePrefix + 'add' + Ext.fontBasePostfix,
            ...
            columns: [{
                    ...
                    renderer: function(value, metadata, record, rowIndex) {
                        ...
                        Ex.defer(function(id, record) {
                            if (Ex.getDom(id)) {
                                var container = Ex.widget('container', {
                                    renderTo: id
                                });
                                var button = Ex.widget('button', {
                                    width: 8,
                                    height: 13,
                                    // ui: 'plain',
                                    // style:'background-color:red',
                                    text: value ? '(' : '',
                                    tooltip: value ? ELocale.buttonText._remove : ELocale.buttonText._add,
                                    handler: function() {
    ...
    validateValueExpression: function() {
        var me = this,
            ...
            ExLocaleExpression = ExLocale.criteriaPanel.validateValueExpression,
            field = me.columns[2].getEditor().value,
        ...
        if (value && field) {
                    ...
                    if (me.validPrefix.contains(parameters[0].trim())) {
                        ...
                        if (actualValueInDisplayStore.length) {
                            ...
                            if (actualValueType !== fieldType) {
                                Ex.Msg.alert(ExLocale.apiName, ExLocaleExpression.typeMismatch);
                                return false;
                            }
                        } else {
                            Ex.Msg.alert(ExLocale.apiName, ExLocaleExpression.valueDoesNotExist(parameters[0].toString()));
                            return false;
                        }
                    } else {
                        if (fieldType !== 0) {
            ...
            if (!field && value) {
                ...
                Ex.Msg.alert(ExLocale.apiName, ExLocaleExpression.selectFieldFirst);
                return false;
            }
        }
        return true;
